#!/bin/bash
#SBATCH -J hard10_sanity
#SBATCH -p gpu
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=120G
#SBATCH -t 04:00:00
#SBATCH -o /mmfs1/scratch/jacks.local/aerfanshekooh/newproject/outputs/logs/hard10_sanity_%j.out
#SBATCH -e /mmfs1/scratch/jacks.local/aerfanshekooh/newproject/outputs/logs/hard10_sanity_%j.err
...
OUT_JSONL="${OUT_JSONL:-$ROOT/outputs/agent_simpledoc_hard10_sanity.jsonl}"
OUT_SUMMARY="${OUT_SUMMARY:-$ROOT/outputs/agent_simpledoc_hard10_sanity_summary.json}"
...
python "$ROOT/m3docrag/examples/run_agent_m3docvqa_subset.py" \
  ...
  --selection-planner-backend heuristic \
  --selection-max-hop-queries 1 \
