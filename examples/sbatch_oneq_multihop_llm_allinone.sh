#!/bin/bash
#SBATCH -J oneq_mhop_all
#SBATCH -p gpu
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=200G
#SBATCH -t 04:00:00
#SBATCH -o /mmfs1/scratch/jacks.local/aerfanshekooh/newproject/outputs/logs/oneq_mhop_all_%j.out
#SBATCH -e /mmfs1/scratch/jacks.local/aerfanshekooh/newproject/outputs/logs/oneq_mhop_all_%j.err

set -euo pipefail

# Self-contained 1-question debug run:
# - Starts local vLLM on the allocated GPU.
# - Runs selection-only multi-hop with answer-conditioned query chaining on CPU.
# - Saves output jsonl + summary.
...
QIDS_ONE="${QIDS_ONE:-$ROOT/outputs/qids_one_debug.txt}"
...
# Keep agent on CPU to avoid GPU OOM when vLLM and retriever share one GPU.
AGENT_DEVICE="${AGENT_DEVICE:-cpu}"

OUT_JSONL="${OUT_JSONL:-$ROOT/outputs/agent_one_multihop_debug.jsonl}"
OUT_SUMMARY="${OUT_SUMMARY:-$ROOT/outputs/agent_one_multihop_debug_summary.json}"
VLLM_STDOUT="${VLLM_STDOUT:-$ROOT/outputs/logs/vllm_inline_oneq_${SLURM_JOB_ID:-manual}.log}"

mkdir -p "$ROOT/outputs/logs"

if [[ ! -s "$QIDS_HARD10" ]]; then
  echo "Missing QIDS_HARD10: $QIDS_HARD10"
  exit 2
fi
...
head -n 1 "$QIDS_HARD10" > "$QIDS_ONE"
echo "Built QIDS_ONE: $QIDS_ONE ($(wc -l < "$QIDS_ONE") qid)"

echo "Using ROOT=$ROOT"
echo "Using BASE_URL=$BASE_URL"
echo "Using AGENT_DEVICE=$AGENT_DEVICE"
...
# Avoid noise from stale shell exports.
unset VLLM_JOB || true
unset VLLM_NODE || true
unset VLLM_PID || true
...
for i in $(seq 1 300); do
  ...
  if ! kill -0 "$VLLM_PID" 2>/dev/null; then
    echo "vLLM exited before readiness. Last logs:"
    tail -n 120 "$VLLM_STDOUT" || true
    exit 3
  fi
  if (( i % 30 == 0 )); then
    echo "Waiting for vLLM... ($i)"
    tail -n 30 "$VLLM_STDOUT" || true
  fi
  sleep 2
done
curl -fsS "$BASE_URL/models" >/dev/null 2>&1 || {
  echo "vLLM not ready after wait. Last logs:"
  tail -n 120 "$VLLM_STDOUT" || true
  exit 4
}
